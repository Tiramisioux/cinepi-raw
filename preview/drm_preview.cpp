...
public:
    DrmPreview(Options const *options,
               std::optional<uint32_t> forced_connector = std::nullopt);
    ~DrmPreview();

    // Display the buffer. You get given the fd back in the BufferDoneCallback
    // once it’s available for re‑use.
    void Show(int fd, libcamera::Span<uint8_t> span,
              StreamInfo const &info) override;

    // Reset the preview window, clearing the current buffers and being ready
    // to show new ones.
    void Reset() override;
...
private:
	...
	bool bind_to_connector(uint32_t connector_id)
	{
		conId_  = connector_id;
		crtcId_ = 0;
		...
		try {
			findCrtc();          // already copes with  conId_  preset
			findPlane();
		} catch (...) {
			return false;        // keep old setup
		}
		Reset();                 // drop old frame‑buffers
		return true;
	}
};
...
void DrmPreview::findCrtc()
{
    ...
    if (!res)
        throw std::runtime_error(
            "drmModeGetResources failed: " + std::string(ERRSTR));

    max_image_width_  = res->max_width;
    max_image_height_ = res->max_height;

	/* the loops below need an index that survives several scopes */
	int i = 0;

    /* ------------------------------------------------------------
     * 2.  Was a connector forced by the caller?
     *      –>  translate it into a CRTC + screen size.
     * ---------------------------------------------------------- */
    if (forced_connector_)
        conId_ = *forced_connector_;          // user‑requested connector id

    if (conId_)
    {
        for (int i = 0; i < res->count_connectors; ++i)
        {
            if (res->connectors[i] != static_cast<uint32_t>(conId_))
                continue;                      // not the one we need

            drmModeConnector *con =
                drmModeGetConnector(drmfd_, conId_);
            if (!con)
                break;                         // fall back to auto‑detect

            if (con->encoder_id)
            {
                drmModeEncoder *enc =
                    drmModeGetEncoder(drmfd_, con->encoder_id);
                if (enc && enc->crtc_id)
                {
                    crtcId_ = enc->crtc_id;

                    if (drmModeCrtc *crtc =
                            drmModeGetCrtc(drmfd_, crtcId_))
                    {
            ...
            break;                             // handled the forced connector
        }
    }

    /* ------------------------------------------------------------
     * 3.  If no connector (or CRTC) determined yet, fall back to
     *     the original “pick first active connector” logic.
     * ---------------------------------------------------------- */
...
// Forward declaration of the real helper that also accepts an
// already‑resolved connector id.
Preview *make_drm_preview(const Options *opts,
	       std::optional<uint32_t> forced_connector = std::nullopt);
	
	/* Compatibility wrapper kept for preview/preview.cpp.  It looks at the
	 * generic Options::hdmi_port value, translates HDMI‑port → DRM connector,
	 * and delegates to the helper above. */
Preview *make_drm_preview(const Options *opts)
{

int port = opts->hdmi_port;        // −1  → “no explicit request”

auto connector = drm_connector_id_for_port(port);

LOG(1, "HDMI request " << port
<< ", selected connector "
<< (connector ? static_cast<int>(*connector) : -1));

return new DrmPreview(opts, connector);
}
